...
- `get_rules()`'s `provider_name` field is the raw normalized alias string (e.g. `"janesmithdo"`), not a human-readable name — confirmed live against a properly seeded database: `GET /providers/prov_1002/rules` returns `"provider_name": "janesmithdo"`, while the sibling `GET /ehr/providers/prov_1002` correctly returns `"name": "Jane Smith, DO"` for the exact same provider. The frontend would display the ugly machine-normalized string, not the actual name.

- The `len(steps) <= 1` fallback in parse_steps() exists at all because of a "boundary consumption" bug in the primary STEP_RE regex: its non-greedy capture group swallows the punctuation the *next* step's own match requires as a prefix boundary (`[\n.:;]`). Isolated and confirmed live: for demo2's text ("...Step 1: Call office. Step 2: Email referrals@clinic.example"), the primary regex alone finds exactly one match — `['Call office.']` — completely missing "Step 2" as a candidate at all, which is what forces `len(steps) <= 1` to be true and the buggy fallback to run.

- For a rule with 3 or more "Step N:" markers, that same boundary-consumption bug means only ODD-numbered steps ever survive the primary regex — even-numbered steps are silently and completely dropped, not just mislabeled. Confirmed live with a 4-step test rule ("Step 1: Call office. Step 2: Email support. Step 3: File report. Step 4: Close ticket."): the primary regex returns only `['Call office.', 'File report.']` — Steps 2 and 4 vanish with zero indication. Since 2 matches doesn't satisfy `len(steps) <= 1`, the fallback never even triggers to rescue them — for a rule like this, roughly half of the actual instructions simply never reach the caller.

- The alias lookup query in get_rules() (`SELECT normalized_alias FROM provider_alias WHERE provider_id = %s`) has no ORDER BY clause, so if a provider ever has more than one alias row, which one becomes `aliases[0]` — used for both the (broken) `provider_name` field and the vector-search query text — isn't guaranteed to be consistent between requests; it depends on Postgres's unspecified row-return order.

- parse_steps() only ever recognizes bullets when they're mixed with an explicit "Step N:" label — a purely bulleted rule with no literal word "Step" anywhere (e.g. "- Call patient\n- Verify insurance\n- Schedule follow-up") produces no steps at all. Confirmed live: `parse_steps()` returns `None` for that input, so the frontend falls back to rendering it as one plain paragraph instead of a list.

- Likewise, parse_steps() requires the literal word "Step" before a number — a naturally-formatted plain numbered list like "1. Call office. 2. Escalate if needed. 3. Document outcome." (without the word "Step") also produces no steps at all. Confirmed live: `parse_steps()` returns `None` for that input too, even though this is an extremely common way to write sequential instructions.

- index.html's favicon references `/vite.svg`, but no such file exists anywhere in the project — confirmed live two ways: the dev server serves back `index.html` itself (`Content-Type: text/html`) for that path since nothing else matches, and the production `dist/` output from `vite build` contains no `vite.svg` at all (just `assets/` and `index.html`), so it would be a genuine 404 in any real deployment. It's leftover from the default Vite scaffold and was never replaced or removed.

- docker-compose.yml pins `postgres:16`, and the Dockerfiles pin `python:3.11-slim`/`node:20-alpine`, but the `adminer` service has no tag at all — it implicitly resolves to `:latest`, the only unpinned image anywhere in the compose file.

- `faiss` (the `faiss-cpu` PyPI package) isn't listed in requirements.txt at all — confirmed by inspecting the file directly, it has exactly 10 lines and none of them is faiss. This means a completely clean `pip install -r requirements.txt`, exactly as the README's Quick Start instructs, can never make `FAISS_AVAILABLE` true — independent of, and in addition to, the ingest.py/main.py file-format mismatch already documented. Vector search is unreachable for two separate, independent reasons.

- Unlike main.py, which falls back to a sensible default DSN (`postgresql://app:app@localhost:5432/rules`) if `PG_DSN` isn't set, ingest.py has no such fallback — `PG_DSN = os.getenv("PG_DSN")`, which is `None` if unset. Confirmed live: passing `None` straight to `psycopg2.connect()` doesn't raise a clear "PG_DSN required" error; it silently falls back to libpq's own defaults and attempts a local Unix-socket connection (`/tmp/.s.PGSQL.5432` under the current OS user), which could just as easily connect to a completely unrelated local Postgres instance instead of failing clearly.

- `ehr_providers_search` builds its SQL LIKE pattern via `f"%{q}%"` with no escaping of LIKE's own wildcard characters — a search query containing a literal `%` or `_` (e.g. part of an NPI-like string or an abbreviation) is interpreted as a wildcard instead of a literal character, silently broadening the match rather than searching for the exact text the user typed.

- RulesPanel.tsx's fetch effect has no cancellation guard at all, unlike ProviderSearch.tsx's `cancelled` flag pattern — if a user selects provider A then quickly switches to provider B before A's request resolves, there's nothing to stop A's later-arriving response from overwriting B's rules with `setRules(...)`, showing the wrong provider's rules with no indication anything's stale.
